...
이전 단계에서 생성한 선호도 데이터셋으로 VAE 모델(인코더 `q_ψ`와 디코더 `r_φ`)을 학습시킵니다. 학습 과정은 Weights & Biases를 통해 기록됩니다.
...
*학습된 모델 (`model.pt` 등)이 `logs/` 디렉토리 내부에 저장됩니다.*
...
python scripts/run_interactive_adaptation.py \
    ...
    --num_queries 10
```
*이 스크립트는 최종적으로 적응된 잠재 벡터를 담은 `adapted_z.pt` 파일을 출력합니다.*
...
```
*이 스크립트는 새롭게 정의된 보상 함수를 후속 강화학습이나 추가 분석에 사용할 수 있도록 하는 기반을 제공합니다.*
...
- **`src/`**: 기능별로 정리된 모든 핵심 소스 코드.
  ...
  - `models/`: VAE 모델 아키텍처 (`vae.py`).
  - `data/`: 데이터 로딩 및 처리 로직 (`loader.py`).
  - `utils/`: 학습, 시각화, 시뮬레이션을 위한 유틸리티 함수.
